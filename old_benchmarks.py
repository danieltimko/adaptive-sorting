...
from old_plotting import plot_results, plot_minrun_results
...
def benchmark_runs_vs_comparisons(reps, arr_size, skip_factor=None, bounds=None):
    ...
    plot_results(results, "# of runs", "# of key comparisons",
                 ...
                 "size_vs_comparisons", fit_to_poly=False)
    return results


def benchmark_runs_vs_cpu_time(reps, arr_size, skip_factor=None, bounds=None):
    if not bounds:
        bounds = (0, arr_size*10)
    results = {}
    for n_runs in range(1, arr_size//2):
...
def run_all_benchmarks():
    ...
    benchmark_runs_vs_cpu_time(1, 50_000, skip_factor=100)
